  ...
  describe '#import' do
    context 'when the file contains valid products' do
      ...
      it 'creates a new product with taxon' do
        expect { product_import.import }
          .to change { Spree::Product.count }.by(1)
          .and change { Spree::Taxon.count }.by(1)
        expect(product_import.row_count).to eq(1)
        expect(product_import.errors).to be_empty
    ...
    context 'when the file contains valid and invalid products' do
      ...
      it 'creates products from valid rows and generates error for invalid rows' do
        expect { product_import.import }
          ...
          .and change { Spree::Taxon.count }.by(1)
        expect(product_import.row_count).to eq(4)
        expect(product_import.errors.count).to eq(2)
        expect(product_import.errors.full_messages).to eq([
